...
class MonthSummaryCalculatorTests(TestCase):
    ...
    def test_provides_list_of_available_months(self):
        create_record('First Record', 100.00, timezone.make_aware(datetime.datetime(2016, 12, 21)))
        create_record('Second Record', -300.00, timezone.make_aware(datetime.datetime(2016, 12, 10)))
        create_record('Third Record', 50.00, timezone.make_aware(datetime.datetime(2016, 11, 8)))
        create_record('Fourth Record', 50.00, timezone.make_aware(datetime.datetime(2017, 1, 3)))
        create_record('Fourth Record', 50.00, timezone.make_aware(datetime.datetime(2017, 1, 1)))
                
        months = MonthSummaryCalculator().listOfAvailable()
        self.assertEquals(3, len(months))
        self.assertEquals('/reports/month-summary/2016/11', months[2]['url'])
        self.assertEquals('Listopad 2016', months[2]['label'])
        self.assertEquals('/reports/month-summary/2016/12', months[1]['url'])
        self.assertEquals('Prosinec 2016', months[1]['label'])
        self.assertEquals('/reports/month-summary/2017/01', months[0]['url'])
        self.assertEquals('Leden 2017', months[0]['label'])
